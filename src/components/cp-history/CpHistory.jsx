...
const CpHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // Fetch client data for each commission holder and combine with history data
        const historyWithData = await Promise.all(historyData.map(async (commissionHolder) => {
          const clientDataResponse = await fetch(`${import.meta.env.VITE_BASE_URL}/history/getPraticularCommissionHolderHistory?commission_holder_id=${commissionHolder.commission_holder_id}`, {
            headers: {
              "Authorization": `Bearer ${accessToken}`,
            },
          });
          ...
          // Combine commission holder data with client data
          return {
            ...commissionHolder,
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const zip = new JSZip();
      const mainFolder = zip.folder("CP_HISTORY_DATA");
      ...
      for (let i = 0; i < history.length; i++) {
        ...
        mainFolder.file(`CP_HISTORY_DATA_OF_${row.commission_holder_name}.csv`, csvContent);
      }

      // Generate the ZIP file and trigger download
      const zipBlob = await zip.generateAsync({ type: "blob" });
      saveAs(zipBlob, "CP_HISTORY_DATA.zip");
    ...
    const handleDownloadCSV = async () => {
      try {
        ...
        saveAs(blob, `CP_HISTORY_OF_CHANNEL_PARTNER_${selectedRowData.commission_holder_name}.csv`);
